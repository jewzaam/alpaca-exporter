...
class MockAlpacaServer:
    ...
    def add_device(self, device_type, device_number, online=True):
        ...
        self.devices[key] = {"online": online, "attributes": {"name": f"Test{device_type.capitalize()}{device_number}"}}
    ...
    def handle_request(self, device_type, device_number, attribute):
        ...
        if error_key in self.error_responses:
            return {"Value": None, "ErrorNumber": self.error_responses[error_key], "ErrorMessage": f"Error {self.error_responses[error_key]}"}

        # Check if attribute exists
        if attribute in self.devices[device_key]["attributes"]:
            return {"Value": self.devices[device_key]["attributes"][attribute], "ErrorNumber": 0, "ErrorMessage": ""}

        # Attribute not found
        return {"Value": None, "ErrorNumber": 1024, "ErrorMessage": "Not implemented"}
